...
# Import apenas para type checking, não em runtime
if TYPE_CHECKING:
...
class CityFilterStrategy(FlightFilterStrategy):
    ...
    def filter(self, flights: List["Flight"]) -> List["Flight"]:  # String type hint
        result = flights
...
class DepartureDateFilterStrategy(FlightFilterStrategy):
    """
    Estratégia para filtrar voos por data de partida.
    """
...
class FlightIdFilterStrategy(FlightFilterStrategy):
    """
    Estratégia para filtrar voo específico por ID.
    """
